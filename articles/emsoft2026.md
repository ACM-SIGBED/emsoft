# EMSOFT 2026: 26th International Conference on Embedded Software
Bonakdarpour, Borzoo | Chang, Li-Pin
* At: Barcelona, Spain
* Published: TBA

## Real-Time Scheduling, Timing, and Control

### Stability and Performance of Control Systems under Probabilistic Logical Execution Time
Sinnema, Yde | Bini, Enrico | Maggio, Martina
* DOI: TBA

### Comparing Controller Synthesis Methods with Deadline-Miss Awareness
Gallant, Melanie | Seide, Marc | Pazzaglia, Paolo | Mandrioli, Claudio | Mark, Christoph | Schmidt, Kevin | Allgöwer, Frank | Maggio, Martina
* DOI: TBA

### Understanding the Pitfalls of a Differentially Private Fixed-Priority Real-Time Scheduler
Mohammad Babar, Fakhruddin | Ahmed, Tamim | Hasan, Monowar
* DOI: TBA

### The zeroLET Task Model and its Application to Offset Design Space Exploration
Wang, Shumo | Bini, Enrico | Deng, Qingxu | Maggio, Martina
* DOI: TBA

### Switch Precision Where It Pays: An Optimal Scheduling Approach for Efficient Control
Banerjee, Debarpita | Lohar, Debasmita | Ghosh, Sumana
* DOI: TBA

## Formal Verification of Cyber-Physical and Hybrid Systems

### Verified Numerical Semantics and Code Extraction for Simulink
Qi, Yuzhen | Wang, Shuling | Jin, Xiangyu | Xu, Xiong | Zhan, Naijun
* DOI: TBA

### Tolerant Barrier Certificates for Stochastic Systems
Feng, Shenghua | Su, Han | Wu, Hao | An, Jie | Chen, Mingshuai | Zhan, Naijun
* DOI: TBA

### A Model Checker for Bounded Hybrid Games
Hamarneh, Qais | Schwammberger, Maike
* DOI: TBA

### Synthesising State-Sensitive Safe Weakly Hard Specifications for Hybrid Dynamical Systems
Adhikary, Sunandan | Bhattacharya, Akash | Dey, Soumyajit
* DOI: TBA

## Verified Software, Model Extraction, and Formal Code Generation

### SLIME: Structural Low-Level Implementation Model Extraction
Hakert, Christian | Günzel, Mario | Chen, Jian-Jia | Craciunas, Silviu S.
* DOI: TBA

### SECO: Towards Code Compaction for Simulink via Verified Equivalence Extraction
Yu, Zehong | Su, Zhuo | Yang, Yixiao | Qiu, Haowei | Wang, Rui | Jiang, Yu
* DOI: TBA

### Specification-Guided Abstraction for Efficient Probabilistic Model Checking
Matsumoto, Tsubasa | Watanabe, Kazuki | Waga, Masaki
* DOI: TBA

### Bridging SystemC and QEMU for Automated FMI/Based Virtual Platform Construction
Albu, Andrei Mihai | Soldi, Dario | Autieri, Fabio | Macii, Enrico | Vinco, Sara
* DOI: TBA

### Modular Extraction of Lustre Models from C Reactive Programs using Frama-C
Junke, Christophe | Siron, Fabien | Correnson, Loïc
* DOI: TBA

## Embedded Systems Performance, Energy, and Resource Management

### Lightweight Asynchronous I/O Optimization for SQLite Scan Performance on Embedded Devices
Lee, Dohwan | Shin, Yewon | Kim, Wook-Hee | Park, Jonghyeok
* DOI: TBA

### A centralized performance monitoring architecture for efficient event processing
Jafri, Mohammed Sajjad | Rahman, Abdur | Sarkar, Emon | Hassen, Mahdi | Thayyil, Gopishankar | Mani, Ashwin Krishna | Pellizzoni, Rodolfo
* DOI: TBA

### A Power Model of System Clocks to Minimize Energy Consumption on Constrained MCUs
Rottleuthner, Michel | Schmidt, Thomas C. | Wählisch, Matthias
* DOI: TBA

### PHiSCo: Predictable, A |High-Performance,  | Scalable and Coherent System, Memory
Hossam, Mohamed | Hassan, Mohamed
* DOI: TBA

### Achieving Speculative Intermittent Computation on Commodity Energy Harvesting Hardware
Zhang, Yida | Huang, Shao-Yu | Min, Byounguk | Park, Andrew Inho | Fang, Gan | Choi, Jongouk | Jung, Changhee
* DOI: TBA

## Edge AI and Autonomous Driving

### DynaStitch: Dynamic ViT Inference via Neural Stitching for Resource-Constrained Edge Devices
Kim, Jiwon | Mottola, Luca | Lee, Jeho | Park, Seonghoon | Cha, Hojung | Voigt, Thiemo
* DOI: TBA

### AdROD: HyperNetwork-based Adversarially Robust Object Detection for Autonomous Driving
Wu, Yuting | Guo, Dongfang | Luo, Xiangzhong | Song, Qun
* DOI: TBA

### BLADE: Lightweight Fault Resilient Semantic Segmentation on Hardware Accelerators
Ordonez, Jhon | Yang, Chengmo
* DOI: TBA

### RIDE: Runtime-Adaptive Inference Depth for Efficient VLA in Autonomous Driving
Hu, Haibo | Huang, Lianming | Li, Qiao | Guan, Nan | Xue, Chun Jason
* DOI: TBA

### Real-Time Wafer-Level Spatial Virtual Metrology on Embedded Platforms Using Frozen Foundation Models
Kim, Hyunwoo | Lee, Munyoung | Jeon, Seung Hyub | Lee, Kyu Sung
* DOI: TBA

## Efficient AI Scheduling, Acceleration, and Hardware/Algorithm Co-Design

### On the Utility of Exploiting Hidden Knowledge of Third-Party Classifiers for Faster Classification
Abdelzaher, Tarek | Baruah, Sanjoy K. | Burns, Alan | Burns, Alan | Hu, Yigong
* DOI: TBA

### Multicore+GPU Scheduling and Job-Level Batching for Real-Time Spectrum Sensing
Goh, Joseph | Ali, Syed | Carter, Nicholas | Crowder, Sarah | Chakraborty, Samarjit | Ward, Bryan C. | Anderson, James H.
* DOI: TBA

### ORBIT: Operator-driven Reasoning for Branching ISA Extension Design in Edge AI
Han, Yaqi | Yuan, Jingzhuo | Yang, Zhijie | Chen, Renzhi | Zhang, Naitian | Rui, Gong | Shen, Li | Wang, Lei
* DOI: TBA

### PACE: Deployment-Time Scheduling Policy Deadline-Aware, for | Adaptive Inference, Intermittent
Mahdi Gholami, Pouya | Xiang, Mingyuan | Hoffmann, Henry
* DOI: TBA

### Branch and Bound for Relational Verification of Neural Networks
Fukuda, Kota | Zhang, Zhenya | Zhang, Guanqin | Zhao, Jianjun
* DOI: TBA

## Distributed, Networked, and Coordinated Cyber-Physical Systems

### Learning-Guided Integrated Scheduling for ATS-Enabled Time-Sensitive Networking
Long, Cheng | Li, Zonghui | Huang, Yu | Cui, Jiamin
* DOI: TBA

### Event-Triggered Decoupled MARL for Real-Time Swarm UAV Control Under Embedded Constraints
Roh, Seungdoek | Park, Sohee | Choi, Daeseon
* DOI: TBA

### Coherence in Control: Bridging Many-Core Mapping and Routing through Cost Unification
Xiong, Guochu | Luo, Xiangzhong | Liu, Weichen
* DOI: TBA

### Suborbital and Orbital Real-Time Localization of Gamma-Ray Bursts via Utility-Guided Coordination
Wang, Daisy | Htet, Ye | Buhler, Jeremy | Markovic, Filip | Sudvarg, Marion
* DOI: TBA

### Expenditure-Optimal Charging for Fixed-Route Electric Vehicles Under Bounded Tardiness
Mandal, Dipankar | Singh, Arshdeep | Sarkar, Arnab | Mondal, Arijit
* DOI: TBA

## Security, Resilience, and Robustness of CPS

### ENOLA: Linear-Space and Low-Overhead Control-Flow Attestation for Microcontroller-based Systems
Armanuzzaman, MD | Kirda, Engin | Zhao, Ziming
* DOI: TBA

### VERA: A Formal Framework for Firmware-First RAS Verification
Yang, Yeqian | Dai, Hongjun | Yu, Jiageng
* DOI: TBA

### Interval POMDP Shielding for Imperfect-Perception Agents
Scarbro, William | Mangal, Ravi
* DOI: TBA

### Spatial Resiliency in Cyber-Physical Systems
Zhang, Zeyu | Chen, Hongkai | Paoletti, Nicola | Lin, Shan | Smolka, Scott
* DOI: TBA

### Diffusion-Guided Search via Exponential Tilting (DiffTilt): An Application to Falsification of Safety-Critical Systems
Khandait, Tanmay | Biswas, Preetom | Okamoto, Hideki | Hoxha, Bardh | Fainekos, Georgios | Pedrielli, Giulia
* DOI: TBA

## Embedded Software, Storage, OS, and Verified Low-Level Implementation

### SqueezeDisk: Bridging High-Ratio Compression and Low-Amplification Decompression in Block Storage
Zhang, Zhihao | Li, Qiao | Wang, Shiyu | Li, Huiba | Shu, Jiwu | Chen, Kai | Zhang, Yiming
* DOI: TBA

### ALLI/O IDE: Portable Embedded Programming via Action-based Diagram and Automatic Code Generation
Narkthong, Nuntipat | Jariyavajee, Chattriya | Xu, Xiaolin
* DOI: TBA

### Correct-by-construction code generation for Human-Machine Interfaces on heterogeneous safety-critical systems
Grand, Charlie | Carle, Thomas | Paulin, Mathias | Vergnaud, Philippe
* DOI: TBA

### Beyond the Monolithic Scheduler: Lightweight Adaptive Policy Switcher for Process Scheduling Optimization
Wang, Xinbo | Jia, Shian | Huang, Ziyang | Cao, Jing | Song, Mingli
* DOI: TBA

### Improving Aligned Memory Copies of CompCert
Bérard, Alexandre | Bonneau, Benjamin | Boulmé, Sylvain | Monniaux, David
* DOI: TBA

## Timing Semantics, Runtime Monitoring, and Specification Analysis

### JSEC: Jitter-to-Step Execution Contracts for Reproducible Distributed ECU-in-the-Loop Co-Simulation
Lee, Hyunjung | Park, Daejin
* DOI: TBA

### FAR-STL: Filter-Aware Robust Signal Temporal Logic for Online Monitoring of Cyber-Physical Systems
Gupta, Supratim | Chatterjee, Sobhan | Zhan, Naijun | Roop, Partha S.
* DOI: TBA

### Uniform Sampling for Timed Regular Expressions
Barbot, Benoît | Bartocci, Ezio | Basset, Nicolas | Dang, Thao | Gigler, Felix | Ničković, Dejan
* DOI: TBA

### Counterexample Classification for Signal Temporal Logic Specifications
Zhenya, Zhang | Kapoor, Parv | An, Jie | Kang, Eunsuk
* DOI: TBA

### Robust Spline-based Planning for Signal Temporal Logic
Williams, Samuel | Deshmukh, Jyotirmoy V.
* DOI: TBA

